...
package object barneshut {
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      ...
      else this //return the same Fork
    }
  }
  ...
  class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float) {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        case Empty(_, _, _) =>
          // no force - nothing needs to be changed
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val matrixX = ((b.x - boundaries.minX) / sectorSize).toInt.max(boundaries.minX.toInt).min(sectorPrecision - 1)
      val matrixY = ((b.y - boundaries.minY) / sectorSize).toInt.max(boundaries.minY.toInt).min(sectorPrecision - 1)
